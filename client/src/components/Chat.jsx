...
const Chat = ({ chatHistory, setChatHistory }) => {
  ...
  const handleSendMessage = async (input) => {
    ...
    try {
      ...
      const finalHistory = updatedHistory.concat({ 
        ai: response.data.answer || response.data.response || "No response received" 
      });
  ...
  // Filter out summary messages for chat display (summary is shown in MainPanel)
  const chatMessages = chatHistory.filter(msg => !msg.summary && !msg.temp);

  // Determine which empty state message to show
  const showUploadMsg = !chatHistory.some((msg) => msg.summary);
  const showStartMsg = !showUploadMsg && !chatMessages.some((msg) => msg.user);
            ...
            <div className="relative z-10 p-4 rounded-lg">
              {chatMessages.map((message, index) => (
                <div key={index} className="mb-2">
                  {message.user && (
                    <div className='text-right w-full grid justify-items-end justify-end'>
                      <p className="bg-gray-700 w-fit rounded-lg px-4 py-3">{message.user}</p>
                    </div>
                  )}
                  {message.ai && (
